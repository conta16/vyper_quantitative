...
class Stmt(object):
    ...
    def __init__(self, stmt, context):
        ...
        self.stmt_table = {
            ...
            vy_ast.While: self.parse_while, #adding while statement to table
    ...
    def parse_while(self):
        print("parse_while")
           ...
           #TODOW: allow looping only if test is initialized
        ...
        with self.context.make_blockscope(block_scope_id):
            ...
            body = ['while', test_expr, parse_body(self.stmt.body, self.context)]
            o = LLLnode.from_list(
                body,
                typ=None, pos=getpos(self.stmt)
            )
...
def parse_body(code, context):
    ...
    ob = LLLnode.from_list(o, pos=getpos(code[0]) if code else None)
    return ob
